...
OpMeta *Reduction::init_task(Task const *task,
  ...
  meta->transformer_layer_id = reduct->layer_guid.transformer_layer_id;
...
void Reduction::forward_task(Task const *task,
  ...
  std::string inputFileName = "check_reduction" + std::to_string(m->transformer_layer_id) + "_" + std::to_string(task->index_point.point_data[0]) + "_input.txt";
  std::string outputFileName = "check_reduction" + std::to_string(m->transformer_layer_id) + "_" + std::to_string(task->index_point.point_data[0]) + "_output.txt";
