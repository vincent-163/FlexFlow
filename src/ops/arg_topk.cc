...
OpMeta *ArgTopK::init_task(Task const *task,
  ...
  m->transformer_layer_id = topk->layer_guid.transformer_layer_id;
  return m;
}
...
InferenceResult
  ...
  std::string inputFileName = "check_arg_topk" + std::to_string(m->transformer_layer_id) + "_" + std::to_string(task->index_point.point_data[0]) + "_input.txt";
  // std::string outputFileName = "check_arg_topk" + std::to_string(m->transformer_layer_id) + "_output.txt";
